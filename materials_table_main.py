...
from MaterialsTable5 import Ui_materials_table
#from integrated2 import Ui_integrated

#to run with sample data: enter "sample_data.txt" into one of the file name cells of the ui and click "done"
#so far, it only works with local data

class materials_table(QtWidgets.QMainWindow):
    def __init__(self):
        super(materials_table, self).__init__()
        self.ui = Ui_materials_table()
        #self.ui = Ui_integrated()
    ...
    def get_vars(self):
        ...
        #
        #atomicMasses are in 2nd row
        atomicMasses = [self.ui.tableWidget.item(0,2).text(),self.ui.tableWidget.item(1,2).text(),
        self.ui.tableWidget.item(2,2).text(), self.ui.tableWidget.item(3,2).text()]
        #
        #atomicFractions are in 3rd row
        atomicFractions = [self.ui.tableWidget.item(0,3).text(),self.ui.tableWidget.item(1,3).text(),
        self.ui.tableWidget.item(2,3).text(), self.ui.tableWidget.item(3,3).text()]
        #
        #densities are in 4th row
        densities = [self.ui.tableWidget.item(0,4).text(),self.ui.tableWidget.item(1,4).text(),
        self.ui.tableWidget.item(2,4).text(), self.ui.tableWidget.item(3,4).text()]
        #
        #thicknesses are in 5th row
        thicknesses = [self.ui.tableWidget.item(0,5).text(),self.ui.tableWidget.item(1,5).text(),
        self.ui.tableWidget.item(2,5).text(), self.ui.tableWidget.item(3,5).text()]

        #components are in 6th row
        components = [self.ui.tableWidget.item(0,6).text(),self.ui.tableWidget.item(1,6).text(),
        self.ui.tableWidget.item(2,6).text(), self.ui.tableWidget.item(3,6).text()]
    ...
    def get_data(self):
        ...
        data_3 = self.file_reader(fname3)

        fname4 = self.ui.tableWidget.item(3,0).text() + ".txt"
        data_4 = self.file_reader(fname4)

        fname5 = self.ui.tableWidget.item(4,0).text() + ".txt"
        data_5 = self.file_reader(fname5)
